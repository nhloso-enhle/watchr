...
export default function TitleCard({ title, mode = 'explore', onTitleClick, animDelay = 0 }) {
  const {
    isInWatchlist, addToWatchlist, removeFromWatchlist, updateItem, getItem,
  } = useWatchlist();
  const [adding, setAdding] = useState(false);

  // Support both TMDB normalised shape and stored watchlist shape
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ width: 52, height: 78, background: 'var(--bg-alt)', flexShrink: 0 }}
      >
        {imageUrl ? (
          <img
            src={imageUrl}
        ...
        {mode === 'explore' ? (
          ...
          <div className="flex items-center gap-1">
            <select
              ...
              className="field"
              style={{ padding: '5px 7px', fontSize: '0.72rem', borderRadius: '8px', width: 'auto', maxWidth: '120px', minHeight: '36px' }}
            >
            ...
            <button
              ...
              style={{
                width: 32, height: 32, borderRadius: 8, border: `1px solid ${isFavourite ? 'rgba(217,119,6,0.35)' : 'var(--border)'}`,
