...
function SkeletonItem() {
  return (
    <div className="card flex items-center gap-3 p-2.5" style={{ borderRadius: '12px' }}>
      ...
      <div className="flex-1 space-y-2"><div className="skeleton h-3 w-3/5" /><div className="skeleton h-2.5 w-2/5" /></div>
  ...
  return (
    <div className="min-h-screen page px-4 sm:px-6 py-8 max-w-3xl mx-auto" style={{ background: 'var(--bg)' }}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="font-bold mb-0.5" style={{ fontSize: '1.65rem', color: 'var(--text)', letterSpacing: '-0.02em' }}>My Watchlist</h1>
          <p className="text-sm" style={{ color: 'var(--text-3)' }}>{stats.total} title{stats.total !== 1 ? 's' : ''} saved</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            <SortAsc size={13} style={{ color: 'var(--text-3)' }} />
            <select value={sort} onChange={e => handleSort(e.target.value)} className="text-sm outline-none bg-transparent cursor-pointer" style={{ color: 'var(--text)', fontFamily: 'Roboto, sans-serif' }}>
              {SORTS.map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
            </select>
          </div>
          <Link to="/explore">
            <button className="btn flex items-center gap-1.5 text-sm font-medium" style={{ background: 'var(--accent)', color: 'var(--accent-fg)', border: 'none', padding: '8px 14px' }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={label} className="card flex items-center gap-3 p-3">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: `color-mix(in srgb, ${color} 12%, transparent)`, color }}>
              <Icon size={14} />
            </div>
            <div>
      ...
      <div className="flex flex-wrap gap-1 mb-6 p-1 rounded-xl w-fit" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        {TABS.map(tab => (
          <button key={tab.key} onClick={() => handleTab(tab.key)}
            ...
            style={{ background: activeTab === tab.key ? 'var(--accent)' : 'transparent', color: activeTab === tab.key ? 'var(--accent-fg)' : 'var(--text-2)' }}>
            {tab.label}
          </button>
        ))}
      </div>

      {loading && <div className="flex flex-col gap-2">{Array.from({ length: 8 }).map((_, i) => <SkeletonItem key={i} />)}</div>}

      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center py-20 text-center">
          <ListVideo size={40} className="mb-3 opacity-20" style={{ color: 'var(--text-3)' }} />
          <p className="font-semibold text-lg mb-1.5" style={{ color: 'var(--text)' }}>{items.length === 0 ? 'Nothing saved yet' : 'No titles match this filter'}</p>
          <p className="text-sm mb-5" style={{ color: 'var(--text-3)' }}>{items.length === 0 ? 'Head to Explore to start building your list' : 'Try a different filter'}</p>
          {items.length === 0 && <Link to="/explore"><button className="btn btn-primary"><Compass size={14} /> Explore titles</button></Link>}
        </div>
      )}
      ...
      {!loading && paged.length > 0 && (
        <>
          <div className="flex flex-col gap-2">
            {paged.map((item, i) => (
              <TitleCard key={item.titleId} title={{ ...item, id: item.titleId }} mode="watchlist" animDelay={i * 20} onTitleClick={id => setModalId(id)} />
            ))}
          </div>
