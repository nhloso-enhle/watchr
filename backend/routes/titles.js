...
// Axios instance with TMDB Bearer token
...
// Normalise TMDB movie/TV objects into a consistent shape
function normalise(item) {
  ...
  return {
    ...
    rating: item.vote_average ? {
      aggregateRating: Math.round(item.vote_average * 10) / 10,
...
// Genre ID → name map (TMDB uses numeric IDs in search results)
const GENRE_MAP = {
...
/* ── GET /:id — full detail (e.g. "movie-550" or "tv-1399") ── */
router.get('/:id', protect, async (req, res) => {
  try {
    const [mediaType, tmdbId] = req.params.id.split('-');
    if (!mediaType || !tmdbId) return res.status(400).json({ message: 'Invalid ID format. Use movie-{id} or tv-{id}' });
